...
class AndCriteria : public Criteria {
   public:
    ...
    virtual std::shared_ptr<std::list<Person>> MeetCriteria(
        std::shared_ptr<std::list<Person>> persons) override {
        std::shared_ptr<std::list<Person>> first_criteria_persons =
            criteria_->MeetCriteria(persons);
